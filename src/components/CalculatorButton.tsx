...
export default function CalculatorButton({
  ...
  if (variant === "primary") {
    ...
    className += ` inset-shadow-shadow-secondary bg-bg-key-secondary text-xl ${theme === "1" ? "text-text-main" : "text-text-secondary"}`;
  } else if (variant === "tertiary") {
    className +=
      " inset-shadow-shadow-accent bg-bg-accent text-xl text-text-secondary-inverse";
  }
    ...
    switch (label) {
      ...
      default: {
        ...
        if (typeof label === "number" && hasLeadingZero(expression)) {
          ...
          isOperator(label as string) &&
          isOperator(topSymbol as string)
        ) {
          nextExpression[nextExpression.length - 1] = label;
        } else {
  ...
  return (
    <button className={className} style={inlineStyles} onClick={handleClick}>
      {label}
    </button>
  );
}
